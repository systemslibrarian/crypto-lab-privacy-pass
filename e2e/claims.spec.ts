...
test('private blind issuance redeems but gives the colluding ledgers no match', async ({ page }) => {
  ...
  // The KAT counter is measured from the corpus the unit suite iterates. The total it publishes
  // has to be the two parts it prints, not a number typed once and left behind.
  const kat = ((await page.locator('[data-claim="kat-count"]').textContent()) ?? '').match(/^(\d+) vectors: (\d+) [^+]+\+ (\d+)/)
  expect(kat, 'the known-answer line prints a total and the two corpora it came from').not.toBeNull()
  await expectClaim(page, 'kat-count', { value: Number(kat?.[2]) + Number(kat?.[3]) })
...
test('valid proofs under client-specific keys redeem and reveal partitioning', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '1. Issue token' }).click()
  await expectVerdict(page, 'dleq', { text: 'DLEQ PROOFS · BOTH VERIFIED', result: 'good' })
  await expectVerdict(page, 'negative-claim', { text: 'does not show it published the same key to every client', result: 'alarm' })
  await page.getByRole('button', { name: '2. Redeem at origin' }).click()
  await expectVerdict(page, 'redemption', { text: 'REDEMPTIONS · BOTH VERIFIED', result: 'good' })
  // The oracle counts the key buckets ITSELF, off the ledger the origin printed, and then requires
  // one drawn line per bucket. A literal 2 on both sides could not tell "one line per partition"
  // from "always two lines", which is the distinction the verdict beside it claims.
  ...
  expect(buckets, 'the partition fixture redeems two tokens under two different key ids').toBe(2)
  await page.getByRole('button', { name: '3. Try to link ledgers' }).click()
  await expectVerdict(page, 'linkage', { text: 'PROOFS VERIFIED · AND PARTITIONED', result: 'alarm' })
  await expectVerdict(page, 'link-map', { text: 'Alice key bucket · Bob key bucket', result: 'alarm' })
