...
function render(): void {
  $('#app').innerHTML = `
    ...
    <section class="comparison blind-rsa-panel" id="blind-rsa" aria-labelledby="blind-rsa-title"><p class="eyebrow">THE OTHER TOKEN TYPE</p><h2 id="blind-rsa-title">The Other Token Type: Blind RSA and &#8730;eNFS (2026)</h2><p>Privacy Pass has two issuance types: <strong>0x0001 VOPRF</strong>, privately verifiable and used by this lab, and <strong>0x0002 Blind RSA</strong>, publicly verifiable (<a href="https://www.rfc-editor.org/rfc/rfc9578#section-8.2.1">RFC 9578 &#167;8.2.1</a>, <a href="https://www.rfc-editor.org/rfc/rfc9578#section-8.2.2">&#167;8.2.2</a>). The paper names Blind RSA deployments including Apple Private Access Tokens, Fastly, Persona, Cloudflare, and iCloud Private Relay (<a href="https://eprint.iacr.org/2026/2131.pdf">Shea et al., IACR ePrint 2026/2131, &#167;7.2</a>).</p><div class="blind-rsa-grid"><div><h3>Why it is exposed</h3><p>A Blind RSA issuer is a raw RSA oracle on blinded values. Queries made for the attack are indistinguishable from honest issuance queries, which is exactly the phase-1 access that &#8730;eNFS needs. Lysyanskaya proved RSABSSA secure under one-more RSA, but Shea et al. argue that assumption does not capture this delayed-target RSA attack (<a href="https://www.rfc-editor.org/rfc/rfc9474#section-7">RFC 9474 &#167;7</a>; <a href="https://eprint.iacr.org/2026/2131.pdf">ePrint 2026/2131, &#167;7.2</a>).</p></div><div><h3>The paper's scale</h3><p><strong>Measured 1024-bit result:</strong> 1,380 CPU-years over five months and <strong>2<sup>32</sup></strong> oracle queries. <strong>At 2048 bits,</strong> the paper extrapolates that the attack needs about <strong>2<sup>90</sup></strong> computation and <strong>2<sup>43</sup></strong> queries: approximately <strong>2<sup>38</sup></strong> CAPTCHAs at 30 tokens each. Cloudflare's more than 2<sup>43</sup> total HTTP requests per day are an infrastructure comparison, not available Blind RSA signing queries. The paper's hypothetical scale calculation assumes Apple's one-per-minute rate limit: about 17 million years for one device, or about 2.3 days across 2.3 billion devices issuing at that rate. At Persona's per-call price, the hypothetical estimate is about $13 trillion. Key-rotation epochs range from hours to weeks (Shea et al., &#167;7.2).</p></div></div><div class="blind-rsa-grid"><div><h3>Mitigations</h3><p>Short term: rotate keys frequently. Medium term: use larger keys. Long term: add zero-knowledge proofs or move to post-quantum schemes. Where public verifiability is unnecessary, use the VOPRF type instead (Shea et al., &#167;7.2).</p></div><div><h3>Scope of this lab</h3><p>This lab does not run the attack. Its VOPRF path is not an RSA oracle, so &#8730;eNFS does not apply to it. For the adjacent RSA signing construction, see the <a href="https://systemslibrarian.github.io/crypto-lab-rsa-forge/#oracle-without-factoring">Crypto Lab RSA Forge &#8730;eNFS section</a>. Blind RSA itself remains outside this implementation.</p></div></div></section>
